...
Grammar ChomskyNormalizer::removeLeftRecursion()
{
    ...
    for (string A: g.getVariables())
    {
        set<vector<string> > recursive_prod;
        set<vector<string> > non_recursive_prod;

        for (auto &prod: g.getProductions(A))
        {
            if (!prod.empty() && prod[0] == A)
            {
                recursive_prod.insert(prod);
            } else
            {
                non_recursive_prod.insert(prod);
            }
        }

        if (recursive_prod.empty()) continue;

        for (auto &prod : recursive_prod)
        {
        ...
        for (auto &prod : non_recursive_prod)
        {
            vector<std::string> aux(prod);
            aux.push_back(A + "'");
            g.addProduction(A, aux);
        }
    }
